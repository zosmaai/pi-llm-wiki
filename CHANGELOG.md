...
### Fixed
- **Layered (personal + project) recall was extension-only; MCP `wiki_recall` silently returned single-vault results** (Issue #131): `recallOperation` called `searchWiki` against the single vault resolved from `WIKI_ROOT`/cwd, so personal-vault pages were never consulted over MCP. The call succeeded and returned `[]`, so nothing signalled that a whole layer had been skipped — the identical query the Pi extension answers with `Found 1 wiki page(s) … (personal + project)`. Fixed by routing `recallOperation` through `searchWikiLayered`, the helper the extension already uses: personal hits are merged in, deduplicated by page ID and tagged `📓 personal`, guarded by `isPersonalVault()` so a personal-root vault is not double-counted, and a no-op when no personal vault exists. `searchWikiLayered` rather than `searchWikiHybrid` on purpose — hybrid degrades to exactly this without an embedder, and wiring one into the MCP server would drag task config and credentials into a process that needs neither. The operation's declared result type is now `RecallResult[]`, making the already-returned `vaultLabel`/`path`/`score` part of the contract. The `wiki_recall` tool description and every README's MCP tool table now state the layering, each translation reusing its own wording from its extension tool table. `test/mcp-parity.test.ts` sandboxes `WIKI_HOME` so results no longer depend on whether the machine running the tests has a personal vault, compares MCP recall against `searchWikiLayered` — the shared contract — and covers the issue's reproduction: an insight saved in the personal vault recalled from a project vault, both layers merging, and no double-count from a personal root.
