...
### Fixed
- **`/wiki-run` prompt frontmatter failed to parse**: the unquoted colon-space in `description` made pi's `yaml.parse` throw `Nested mappings are not allowed in compact mappings`, so the template was dropped and reported as `[Prompt conflicts]`. The description is now a quoted string, and `test/package-structure.test.ts` parses every `prompts/*.md` frontmatter the same way pi does.
